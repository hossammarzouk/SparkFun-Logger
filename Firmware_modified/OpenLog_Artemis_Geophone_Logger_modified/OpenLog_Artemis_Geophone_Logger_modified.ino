...
#define QWIIC_PULLUPS 0 // Default to 1k pull-ups on the Qwiic bus
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//SFE_UBLOX_GPS myGPS;
int Lat = 0; int Long = 0; int Alt = 0 ;int Fix = 0 ; int SIV = 0;
...
//getUbloxDateTime(yy, mm, dd, h, m, s, ms, dateValid, timeValid);

   lcd.setCursor(4,2);  lcd.print(SIV);
   lcd.setCursor(13,2); lcd.print(Fix);  
   
   if (Fix == 3){lcd.setCursor(18,2); lcd.print(":)");
   lcd.setCursor(0,3); lcd.print("....synchronized...."); 
   delay(3000); lcd.clear();
    }
        }

  while(online.dataLogging == true){
  char gpsTime[9]; //09:14:37
  getUbloxDateTime(year, month, day, hour, minute, second, milliseconds, datevalid, timevalid);
   sprintf(gpsTime, "%02d:%02d:%02d", hour, minute, second); //TODO get GPS hundredths()
    lcd.setCursor(0,0); lcd.print("Ready for recording");  
    lcd.setCursor(0,1); lcd.print(gpsTime);  
